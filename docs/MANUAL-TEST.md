...
| Part | Where | Version |
|---|---|---|
| Engine (complete) | branch `core/all-engine-work` @ `151aa89ed41` = local `main` in `~/development/summerengine`; PR #194 (draft, CI only) | 0.5.66 |
...
1. Engine: `git -C ~/development/summerengine push origin 151aa89ed41:main` → `scripts/release/ship_macos.sh 0.5.66 --source-commit 151aa89ed41 --public-base-commit <web sha>` on the signing Mac; Windows via `release.ps1` on the Certum runner.
