          ...
          text: JSON.stringify(
            {
              ...
              verificationLadder: {
                ...
                "4_interactive": [
                  ...
                  "RunVerification: spawn a hidden, disposable game instance that runs a GDScript probe (press inputs, read state, save frames) and dies — never touches the user's editor. Returns results.json + frames. Best for a scripted 'does X happen when I do Y' assertion. save_frame REQUIRES a name argument — save_frame() with no args is a script error (probe fails).",
                  "If neither RunVerification nor SimulateInput is available, do NOT fake the interaction result — ask the user to try it and report what they see.",
                ],
              },
              ...
              rawOpsViaBatch: [
                ...
                "RunVerification (hidden probe instance): summer_batch ops:[{op:'RunVerification', probe_source:'<gdscript>', max_seconds:20}]. probe_source extends SummerProbeBase and uses report(name, value)/save_frame(name)/press(action)/key(keycode)/finish(); returns {ok, results, frames, out_dir} or {ok:false, failure_reason: spawn_failed|timeout|bad_args|no_project}. save_frame REQUIRES a name argument.",
                "SimulateInput (drive the RUNNING game — summer_play first): summer_batch ops:[{op:'SimulateInput', type:'action', action:'jump', pressed:true}], sent ALONE as the only op in the batch. Other shapes: type:'key' with keycode or physical_keycode/pressed; type:'mouse_click' with position:[x,y] and optional button (a complete press+release; no pressed field); type:'axis' with action_negative/action_positive/strength/duration_ms.",
                "SimulateInput failure modes: failure_reason 'not_running' = start the game first; 'unsupported' = the running game build predates the handler — use RunVerification instead; 'unsupported_transport' = it was batched with other ops — resend it as the ONLY op (nothing from the rejected batch was applied).",
              ...
              recovery: [
                ...
                "If a run op returns 'unsupported' / an unknown-op error: this engine build predates it — fall back to summer_play + summer_get_debugger_errors, use RunVerification when available, or ask the user to interact.",
